...
function MovieTrailer({ movieId, className }) {
	...
	if (videos.length < 1) {
		return <h2 className="text-primary text-2xl font-semibold mb-10 text-center">No Videos...</h2>;
	}
		...
		<section className={`${className} flex flex-col gap-14`}>
			{videos.slice(0, 3).map((video) => (
				<div key={video.id} className="text-center">
					<h3 className="text-lg mb-5 bg-primary p-2 inline-block rounded-md">{video.name}</h3>
					<iframe
						className="aspect-video w-full md:w-2/3 mx-auto"
